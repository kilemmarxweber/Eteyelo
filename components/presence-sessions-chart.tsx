...
const EMPTY_STATS: PresenceChartStats = {
  todayCount: 0,
  totalCount: 0,
  series: [],
};

function formatValue(value: number, locale: string, mode: "sessions" | "hours") {
  if (mode === "hours") {
...
export function PresenceSessionsChart({
  mode,
  stats,
  loading,
}: {
  mode: "sessions" | "hours";
  stats?: PresenceChartStats | null;
  loading?: boolean;
}) {
    ...
    >
      <CardHeader className="gap-0.5 space-y-0 border-b border-emerald-100/80 !p-4 !pb-3 dark:border-emerald-900/40">
        <CardTitle className="flex items-center gap-2 text-base font-semibold text-emerald-950 dark:text-emerald-100">
          ...
          {mode === "hours" ? t("chartHoursTitle") : t("chartSessionsTitle")}
        </CardTitle>
        <CardDescription className="text-sm text-emerald-800/80 dark:text-emerald-300/80">
          {mode === "hours"
            ? t("chartHoursSubtitle")
            : t("chartSessionsSubtitle")}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-3 p-4 pt-3">
        <div className="grid grid-cols-2 gap-2">
          <StatBlock
            label={
              mode === "hours" ? t("chartHoursToday") : t("chartSessionsToday")
            }
            value={
              loading
                ? "—"
                : `${formatValue(todayCount, locale, mode)}${mode === "hours" ? " h" : ""}`
            }
            tone="today"
            loading={loading}
          />
          <StatBlock
            label={
              mode === "hours" ? t("chartHoursTotal") : t("chartSessionsTotal")
            }
            value={
              loading
                ? "—"
                : `${formatValue(totalCount, locale, mode)}${mode === "hours" ? " h" : ""}`
            }
            tone="total"
            loading={loading}
          />
        ...
        <div className="rounded-xl border border-emerald-100/80 bg-white/70 px-2 pb-1 pt-2 dark:border-emerald-900/40 dark:bg-emerald-950/20">
          <p className="px-1 text-[11px] font-medium text-emerald-800/80 dark:text-emerald-300/80">
            {mode === "hours" ? t("chartHoursSeries") : t("chartSessionsSeries")}
          </p>
                      ...
                      return (
                        <div className="rounded-lg border bg-popover px-2.5 py-1.5 text-xs shadow-md">
                          ...
                          <p className="text-muted-foreground">
                            {mode === "hours"
                              ? t("chartHoursTooltip", {
                                  count: formatValue(value, locale, "hours"),
                                })
                              : t("chartSessionsTooltip", {
        ...
        <div className="space-y-1.5">
          <div className="flex items-center justify-between text-[11px] text-muted-foreground">
            <span>{t("chartShareLabel")}</span>
            <span className="font-medium text-emerald-800 dark:text-emerald-200">
              {loading ? "—" : t("chartShare", { pct: shareLabel })}
            </span>
          </div>
