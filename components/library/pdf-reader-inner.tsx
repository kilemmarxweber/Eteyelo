...
type PdfReaderInnerProps = {
  bookId: string;
  className?: string;
};

export function PdfReaderInner({ bookId, className }: PdfReaderInnerProps) {
  ...
  const fileUrl = `/api/library/${bookId}/file`;

  if (!workerReady) {
    return (
      <div
        className={cn(
          "flex min-h-[420px] items-center justify-center gap-2 rounded-xl border border-border bg-card text-sm text-muted-foreground shadow-sm",
          className,
        )}
      >
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border bg-muted/40 px-3 py-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            variant="ghost"
            onClick={() => {
              const el = document.getElementById(`pdf-reader-${bookId}`);
      ...
      <div
        id={`pdf-reader-${bookId}`}
        className="flex max-h-[75vh] min-h-[420px] justify-center overflow-auto bg-muted/20 p-4"
      >
        {error ? (
