...
import { isBranchTypePubliclyRegistrable } from "@/lib/branch-capabilities";
...
export async function submitSchoolRegistrationRequestAction(
  ...
  if (!isBranchTypePubliclyRegistrable(parsed.data.typebranch)) {
    return {
      data: null,
      error:
        "Les ateliers doivent être créés au sein d'une branche existante et ne peuvent pas être inscrits via ce formulaire.",
    };
  }
