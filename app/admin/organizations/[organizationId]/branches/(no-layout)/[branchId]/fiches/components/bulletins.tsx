...
export default function BulletinPDF({ data }: { data: any[] }) {
  ...
  const generatePDF = useCallback(() => {
    ...
    function generateFooterBlock(
      ...
      doc.text("Signature de l’élève", startX + 5, startY + lineHeight * 4 + 4);
    ...
    doc.rect(subX, tableY + hTop + hMid + shiftY, sem1PeriodWidths[0], hBottom);
    doc.text(
      "1eP",
    ...
    doc.rect(subX, tableY + hTop + hMid + shiftY, sem2PeriodWidths[0], hBottom);
    doc.text(
      "3eP",
    ...
    // Colonnes finales
    doc.rect(colPos[3] + shiftX, tableY + shiftY, colWidths[3], rowHeightTotal);
    doc.text(
      "TG",
    ...
    doc.rect(colPos[4] + shiftX, tableY + shiftY, colWidths[4], rowHeightTotal);
    doc.text(
      "SIGN PROF",
    ...
    doc.rect(colPos[6] + shiftX + leftWidth, bottomY, rightWidth, frameHeight);
    doc.text(
      "Signature",
    ...
    const selectedPeriod = [...validPeriods]
      .sort(
        (a, b) =>
          periodOrder.indexOf(a.periodName) - periodOrder.indexOf(b.periodName),
      )
    ...
    const allSubjects = new Set<string>(
      student.periods.flatMap((p: StudentPeriod) => Object.keys(p.notes || {})),
    );
    ...
    function drawCell(
      ...
      if (typeof color === "string") {
        ...
        if (color.text && palette[color.text]) textColor = palette[color.text];
        if (color.fill && palette[color.fill]) fillColor = palette[color.fill];
      }
    ...
    blocs.forEach((bloc) => {
      ...
      for (let i = 4; i <= 6; i++) {
        ...
        drawCell1(doc, colPos[i] + shiftX, yPosBlocs, width, maximaHeight, "", {
          isMaxima: true,
        });
      }
