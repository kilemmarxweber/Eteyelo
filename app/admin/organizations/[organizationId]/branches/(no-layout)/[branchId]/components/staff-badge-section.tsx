...
import { getStaffBadgeTitle, type StaffBadgeData } from "@/lib/staff-badge";
import { StaffBadgeCard } from "./staff-badge-card";
...
export function StaffBadgeSection({ badge }: StaffBadgeSectionProps) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      link.download = `carte-${badge.lastName}-${badge.firstName}.png`;
  ...
  async function handlePrint() {
    ...
    try {
      ...
      const cardTitle = getStaffBadgeTitle(badge.kind);
      printWindow.document.open();
      printWindow.document.write(`<!DOCTYPE html>
<html lang="fr">
  <head>
    <meta charset="utf-8" />
    <title>${cardTitle} - ${badge.fullName}</title>
    <style>
      @page { size: auto; margin: 10mm; }
      * { box-sizing: border-box; }
      html, body { margin: 0; padding: 0; background: #ffffff; }
      body { min-height: 100vh; display: flex; align-items: center; justify-content: center; }
      img { display: block; width: ${cardWidth}px; height: ${cardHeight}px; max-width: 100%; object-fit: contain; }
    </style>
  </head>
  <body>
    <img id="staff-card-print" src="${dataUrl}" alt="${cardTitle}" />
    <script>
      const image = document.getElementById("staff-card-print");
      function launchPrint() { window.focus(); window.print(); }
  ...
  const cardTitle =
    badge.kind === "teacher" ? "Carte d'enseignant" : "Carte du personnel";
  ...
  return (
    <div className="rounded-xl border bg-card p-4 shadow-sm">
      ...
      <StaffBadgeCard badge={badge} preview />
      ...
      {mounted
        ? createPortal(
            ...
            >
              <StaffBadgeCard ref={badgeRef} badge={badge} />
            </div>,
            document.body,
          )
        : null}
