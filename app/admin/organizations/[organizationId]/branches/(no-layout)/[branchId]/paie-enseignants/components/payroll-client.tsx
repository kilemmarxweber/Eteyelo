...
export default function PayrollClient() {
  ...
  useEffect(() => {
    if (!hydrated || isPending || !isManager) return;
    if (tab !== "brouillon") return;
    if (!schoolYearId || loading || working) return;
    if (rows.length > 0) return;
    const key = `${schoolYearId}:${year}:${month}`;
    ...
    void (async () => {
      ...
      if (error) {
        ...
        if (result?.missingExchangeRate) {
          toast.warning(t("toast.noRateDraft"));
        }
        toast.success(t("toast.generated", { count: result.count }));
        await load();
      }
      setWorking(false);
    })();
        ...
        <CardContent>
          <div className="mb-3 flex items-center justify-between gap-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                ...
                Enseignants secondaire : brut = montant / séance × séances de l’horaire
                du mois. Changer de mois conserve le net du mois précédent et recalcule
                le nouveau.
              </p>
            </div>
            {isManager ? (
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {([
              ...
              ["secondaryNonMatriculeSessionRate", "Secondaire : montant / séance (× séances horaire du mois)"],
