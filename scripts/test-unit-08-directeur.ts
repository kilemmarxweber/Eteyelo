 ...
 * Même permissions (pédagogie, sans finance) ; libellé selon type d’école.
 */
...
test("prefet ≡ directeur : pedagogy + manage + HR write — pas finance", () => {
...
test("menu chef établissement = pédagogie — pas Finance", () => {
  for (const session of [sessionDirecteur, sessionPrefet]) {
    const links = buildStaticSideLinks(session, BRANCH_PATH, "PRIMAIRE");
    const titles = links.map((item) => item.title);
    assert.ok(!titles.includes("Finance"), "chef école ne doit pas voir Finance");
...
test("libellés contextuels : secondaire Préfet ; autres types Directeur", () => {
  assert.equal(schoolHeadRoleLabel("PRIMAIRE"), "Directeur");
  assert.equal(schoolHeadRoleLabel("SECONDAIRE"), "Préfet");
  assert.equal(schoolHeadRoleLabel("UNIVERSITE"), "Directeur");
  assert.equal(schoolHeadRoleLabel("CENTRE_FORMATION"), "Directeur");
  assert.equal(schoolHeadRoleLabel("ATELIER"), "Directeur");
  assert.equal(
  ...
  assert.equal(
    orgRoleLabel(ORG_ROLE.DIRECTEUR, { typebranch: "UNIVERSITE" }),
    "Directeur",
  );
...
test("distinct du directeur des études : HR write ; les deux sans finance", () => {
  ...
  assert.equal(dirBlocks.pedagogyMetrics, true);
...
test("hub org OK ; post-login → branche d’affectation (sinon /ecodim)", () => {
